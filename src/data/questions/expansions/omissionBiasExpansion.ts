...
const SCENARIOS: ReadonlyArray<readonly [string, string]> = [
  ...
  ['Zoe notices her neighbor left a candle burning and decides not to mention it because it is awkward.', 'Zoé remarque que sa voisine a laissé une bougie allumée et décide de ne rien dire parce que c est gênant.'],
  ['Kai sees the elevator is stuck between floors and waits instead of hitting the emergency button because it feels dramatic.', 'Kai voit que l ascenseur est coincé entre deux étages et attend au lieu d appuyer sur le bouton d urgence parce que cela lui semble dramatique.'],
  ['Nora spots a broken step at the front door and leaves a note for later rather than warning people now.', 'Nora repère une marche cassée à l entrée et laisse un mot pour plus tard au lieu de prévenir les gens maintenant.'],
  ['Eli hears the freezer door is open but lets it go because closing it would mean leaving his seat.', 'Eli entend que la porte du congélateur est ouverte mais ne fait rien parce que la fermer l obligerait à quitter sa place.'],
  ['Lina sees a child wander near the street and does nothing because another adult might notice first.', 'Lina voit un enfant s approcher de la rue et ne fait rien parce qu un autre adulte le verra peut-être avant elle.'],
  ['Grant notices the fire alarm battery is dying and ignores it because replacing it is annoying.', 'Grant remarque que la pile du détecteur de fumée est faible et l ignore parce que la remplacer l embête.'],
  ['Tia sees the wrong address on a package label and does not correct it because it is not her package.', 'Tia voit la mauvaise adresse sur une étiquette de colis et ne la corrige pas parce que le colis ne lui appartient pas.'],
  ['Noah knows the group chat contains the wrong meeting time but says someone else can fix it.', 'Noah sait que le groupe contient la mauvaise heure de réunion mais dit que quelqu un d autre pourra corriger ça.'],
  ['Mia spots a leak under the sink and leaves a bucket there instead of calling maintenance.', 'Mia repère une fuite sous l évier et met un seau dessous au lieu d appeler la maintenance.'],
  ['Jules sees the bike lock is broken and rides anyway because stopping would take effort.', 'Jules voit que le cadenas du vélo est cassé et roule quand même parce que s arrêter demanderait un effort.'],
  ['Ava notices a medicine bottle without a cap and sets it aside without telling anyone.', 'Ava remarque qu un flacon de médicament n a pas de bouchon et le met de côté sans prévenir personne.'],
  ['Finn hears the smoke detector chirp but keeps watching TV because it is only one beep.', 'Finn entend le détecteur de fumée biper mais continue de regarder la télé parce que ce n est qu un bip.'],
  ['Mina sees the oven timer is off by ten minutes and decides not to fix it mid-recipe.', 'Mina voit que la minuterie du four a dix minutes de décalage et décide de ne pas la corriger au milieu de la recette.'],
  ['Theo notices the office door will not latch properly and leaves it because it still mostly closes.', 'Theo remarque que la porte du bureau ne se verrouille pas bien et la laisse ainsi parce qu elle ferme quand même presque.'],
  ['Juno spots a wet floor sign that fell over and walks past it without picking it up.', 'Juno voit qu un panneau de sol mouillé est tombé et passe devant sans le relever.'],
  ['Iris knows the homework answer key has a mistake but stays quiet because correcting it is inconvenient.', 'Iris sait que le corrigé du devoir contient une erreur mais se tait parce que le corriger est gênant.'],
  ['Cole sees a loose railing on the porch and decides not to mention it today.', 'Cole voit une rambarde branlante sur le perron et décide de ne pas le mentionner aujourd hui.'],
  ['Riley notices the online form asks for the wrong birthday and leaves it because the system will probably survive.', 'Riley remarque que le formulaire en ligne demande la mauvaise date de naissance et le laisse ainsi parce que le système survivra probablement.'],
  ['Pia sees a small child playing with a heavy object and does not step in because the parent is nearby.', 'Pia voit un petit enfant jouer avec un objet lourd et n intervient pas parce que le parent est à proximité.'],
  ['Hugo notices the store refund policy is wrong and says the cashier will probably hear about it eventually.', 'Hugo remarque que la politique de remboursement du magasin est fausse et dit que la caissière l entendra probablement un jour.'],
  ['Sam sees the car tire is soft and keeps driving because stopping now would be a hassle.', 'Sam voit que le pneu de la voiture est mou et continue de conduire parce que s arrêter maintenant serait pénible.'],
  ['Jade hears the baby monitor battery is low and decides to plug it in later.', 'Jade entend que la batterie du babyphone est faible et décide de le brancher plus tard.'],
  ['Owen notices the school app sent the wrong notice and assumes another parent will complain.', 'Owen remarque que l application de l école a envoyé la mauvaise annonce et suppose qu un autre parent se plaindra.'],
  ['Lea sees the meeting room booking is wrong and leaves it because changing it might create a fuss.', 'Léa voit que la réservation de la salle de réunion est fausse et la laisse ainsi parce que la changer pourrait faire du bruit.'],
  ['Maya hears the freezer is unplugged by mistake but waits because the food still feels cold.', 'Maya entend que le congélateur a été débranché par erreur mais attend parce que la nourriture semble encore froide.'],
  ['Ben notices the classroom window is open during rain and leaves it because getting up would interrupt the lesson.', 'Ben remarque que la fenêtre de la classe est ouverte sous la pluie et la laisse parce que se lever interromprait la leçon.'],
  ['Ava spots a loose stair at home and tells herself nobody will use it today.', 'Ava repère une marche branlante à la maison et se dit que personne ne l utilisera aujourd hui.'],
  ['Omar sees the website shipping address is wrong and leaves it because it is already late in the day.', 'Omar voit que l adresse de livraison du site est fausse et la laisse parce qu il est déjà tard dans la journée.'],
  ['Nina notices the smoke detector chirps every minute and mutes it instead of replacing the battery.', 'Nina remarque que le détecteur de fumée bippe toutes les minutes et le met en sourdine au lieu de remplacer la pile.'],
  ['Eli sees the medication dosage instruction is wrong and says the pharmacy will fix it next time.', 'Eli voit que l instruction de dosage du médicament est fausse et dit que la pharmacie la corrigera la prochaine fois.'],
  ['Lina hears the floor is wet from a spill and walks past because someone else must have seen it.', 'Lina entend que le sol est mouillé à cause d un renversement et passe devant parce que quelqu un d autre l a sûrement vu.'],
  ['Grant notices a neighbor left the garage open and does not mention it because it feels nosy.', 'Grant remarque qu un voisin a laissé le garage ouvert et ne dit rien parce que cela semble indiscret.'],
  ['Tia sees the class roster has the wrong phone number and leaves it because the office probably has another copy.', 'Tia voit que la liste de classe contient le mauvais numéro de téléphone et la laisse parce que le bureau a probablement un autre exemplaire.'],
  ['Noah spots the garden hose lying across the walkway and steps over it without moving it.', 'Noah repère le tuyau d arrosage qui traverse le passage et enjambe sans le déplacer.'],
  ['Mia knows the emergency exit sign is blocked but does not report it because the hallway is busy.', 'Mia sait que la sortie de secours est bloquée mais ne le signale pas parce que le couloir est occupé.'],
  ['Jules hears the fridge is making a strange noise and ignores it because it still cools the food.', 'Jules entend que le frigo fait un bruit étrange et l ignore parce qu il refroidit encore la nourriture.'],
  ['Ava sees the wrong allergy label on a dish and chooses not to say anything because she is not the cook.', 'Ava voit la mauvaise étiquette d allergène sur un plat et choisit de ne rien dire parce qu elle n est pas la cuisinière.'],
  ['Finn notices the car seat is not strapped in right and decides to leave it because the trip is short.', 'Finn remarque que le siège auto n est pas bien attaché et décide de le laisser ainsi parce que le trajet est court.'],
  ['Mina spots a missing page in the report and shrugs because it is only one page.', 'Mina repère une page manquante dans le rapport et hausse les épaules parce que ce n est qu une page.'],
  ['Theo hears the phone screen protector is cracked and does not replace it because the screen still works.', 'Theo entend que la protection d écran du téléphone est fissurée et ne la remplace pas parce que l écran marche encore.'],
  ['Juno notices the public notice has the wrong date and says someone else will tell the office.', 'Juno remarque que l avis public indique la mauvaise date et dit que quelqu un d autre le dira au bureau.'],
  ['Iris sees the broken umbrella in the hallway and leaves it there because it is not in her way.', 'Iris voit le parapluie cassé dans le couloir et le laisse parce qu il ne gêne pas son passage.'],
  ['Cole notices the stove burner stayed on low and does nothing because the pot has not boiled over yet.', 'Cole remarque que le brûleur du poêle est resté allumé à feu doux et ne fait rien parce que la casserole ne déborde pas encore.'],
  ['Riley hears the alarm system is glitching and says the building is still probably fine.', 'Riley entend que le système d alarme bugue et dit que le bâtiment est probablement encore en bon état.'],
  ['Pia sees the checkout price is wrong and chooses not to challenge it because the difference is small.', 'Pia voit que le prix à la caisse est faux et choisit de ne pas le contester parce que la différence est petite.'],
  ['Hugo notices the school lunch menu lists the wrong allergy information and assumes it will be corrected before lunch.', 'Hugo remarque que le menu scolaire indique la mauvaise information sur les allergies et suppose que ce sera corrigé avant le déjeuner.'],
  ['Sam sees the car windshield is cracked and keeps driving because replacing it is a chore.', 'Sam voit que le pare-brise de la voiture est fissuré et continue à conduire parce que le remplacer est une corvée.'],
  ['Jade hears the neighbors arguing and decides not to check because it is none of her business.', 'Jade entend les voisins se disputer et décide de ne pas vérifier parce que cela ne la regarde pas.'],
];
