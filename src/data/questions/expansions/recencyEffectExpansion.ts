...
const SCENARIOS: ReadonlyArray<readonly [string, string]> = [
  ...
  ['Sophie listens to a long podcast episode, but the closing rant is what she keeps quoting.', 'Sophie écoute un long épisode de podcast, mais la dernière tirade est ce qu elle continue de citer.'],
  ['Liam reads a helpful article, but the last pessimistic sentence is the one he remembers.', 'Liam lit un article utile, mais la dernière phrase pessimiste est celle dont il se souvient.'],
  ['Emma has a good meeting, yet the final awkward pause makes her think the whole session was bad.', 'Emma a une bonne réunion, mais la dernière pause gênante lui fait penser que toute la séance était mauvaise.'],
  ['Diego enjoys most of a documentary, but the ending warning is what he talks about afterward.', 'Diego aime presque tout un documentaire, mais l avertissement final est ce dont il parle ensuite.'],
  ['Lily hears many reassuring updates, but the final delay message is what keeps her worried.', 'Lily reçoit beaucoup de nouvelles rassurantes, mais le dernier message de retard est ce qui continue de l inquiéter.'],
  ['Hassan reads a balanced review, but the last negative line decides it for him.', 'Hassan lit un avis équilibré, mais la dernière ligne négative décide pour lui.'],
  ['Ruby enjoys a class discussion, but the final disagreement is what feels most real to her.', 'Ruby aime une discussion en classe, mais le dernier désaccord est ce qui lui paraît le plus réel.'],
  ['Noah listens to a long explanation, but the last example is the only one he can recall.', 'Noah écoute une longue explication, mais le dernier exemple est le seul dont il puisse se souvenir.'],
  ['Chloe has a nice dinner, but the final awkward goodbye is what she replays in her head.', 'Chloe passe un bon dîner, mais le dernier au revoir gênant est ce qu elle rejoue dans sa tête.'],
  ['Yusuf enjoys a workshop with many useful tips, but the closing criticism is what lingers.', 'Yusuf apprécie un atelier avec beaucoup de conseils utiles, mais la critique finale est ce qui persiste.'],
  ['Maya reads a long product comparison, but the last battery note is the one she trusts.', 'Maya lit une longue comparaison de produits, mais la dernière note sur la batterie est celle qu elle croit.'],
  ['Lucas has a calm flight, but the bumpy landing is the part he remembers most.', 'Lucas a un vol calme, mais l atterrissage cahoteux est la partie dont il se souvient le plus.'],
  ['Ari watches a debate with many good points, but the final voice in the room feels strongest.', 'Ari regarde un débat avec beaucoup de bons arguments, mais la dernière voix dans la salle lui paraît la plus forte.'],
  ['Mina gets many positive comments on her report, but the final caveat is what she focuses on.', 'Mina reçoit beaucoup de commentaires positifs sur son rapport, mais la dernière réserve est ce sur quoi elle se concentre.'],
  ['Owen hears a helpful lecture, but the ending joke is the bit he tells his friends about.', 'Owen entend une conférence utile, mais la blague de fin est ce qu il raconte à ses amis.'],
  ['Zara enjoys most of the road trip, but the final traffic jam becomes the whole trip in memory.', 'Zara apprécie la plupart du road trip, mais le dernier bouchon devient tout le voyage dans sa mémoire.'],
  ['Ben reads a forum thread with plenty of support, but the last complaint is the post he remembers.', 'Ben lit un fil de forum avec beaucoup de soutien, mais la dernière plainte est le message dont il se souvient.'],
  ['Nora listens to a fair proposal, but the final warning is the one that stays with her.', 'Nora écoute une proposition juste, mais le dernier avertissement est celui qui reste avec elle.'],
  ['Eli enjoys a long family gathering, but the last tense exchange is what colors his memory.', 'Eli apprécie une longue réunion de famille, mais le dernier échange tendu colore son souvenir.'],
  ['Pia watches a sports game with many strong plays, but the final mistake is the one she repeats.', 'Pia regarde un match avec beaucoup de belles actions, mais la dernière erreur est celle qu elle répète.'],
  ['Theo gets several reassuring texts, but the final short reply is what he fixates on.', 'Théo reçoit plusieurs messages rassurants, mais la dernière réponse brève est ce sur quoi il se fixe.'],
  ['Juno reads a long helpful review, but the last negative note feels like the biggest issue.', 'Juno lit un long avis utile, mais la dernière remarque négative lui semble être le plus gros problème.'],
  ['Iris has a productive meeting, but the final disagreement is what defines it for her.', 'Iris a une réunion productive, mais le dernier désaccord la définit à ses yeux.'],
  ['Cole enjoys a good day at school, but the final detention announcement is all he talks about.', 'Cole passe une bonne journée à l école, mais l annonce finale de retenue est tout ce dont il parle.'],
  ['Riley hears a podcast with many clear points, but the last dramatic line is what he quotes.', 'Riley écoute un podcast avec beaucoup d idées claires, mais la dernière phrase dramatique est celle qu il cite.'],
  ['Pia gets a good medical checkup, but the final follow-up reminder is what makes her anxious.', 'Pia passe un bon contrôle médical, mais le dernier rappel de suivi est ce qui l inquiète.'],
  ['Hugo likes a neighborhood meeting, but the last complaint becomes the whole story for him.', 'Hugo aime une réunion de quartier, mais la dernière plainte devient toute l histoire pour lui.'],
  ['Sam reads a balanced article, but the closing line about risk is what changes his mind.', 'Sam lit un article équilibré, mais la dernière ligne sur le risque est ce qui change son avis.'],
  ['Jade watches a training video with useful advice, but the final warning is the part she remembers.', 'Jade regarde une vidéo de formation avec des conseils utiles, mais le dernier avertissement est la partie dont elle se souvient.'],
  ['Maya has a pleasant afternoon with friends, but the final awkward silence is what stays with her.', 'Maya passe un après-midi agréable avec des amis, mais le dernier silence gênant est ce qui reste avec elle.'],
  ['Ben hears a story with many positive details, but the ending is the only part he retells.', 'Ben entend une histoire avec beaucoup de détails positifs, mais la fin est la seule partie qu il raconte à nouveau.'],
  ['Ava enjoys a class trip, but the closing delay at the bus stop is what makes her complain.', 'Ava aime une sortie scolaire, mais le dernier retard à l arrêt de bus est ce qui la fait se plaindre.'],
  ['Omar reads a guide with good advice, but the last caution is what he treats as the main point.', 'Omar lit un guide avec de bons conseils, mais la dernière mise en garde est ce qu il traite comme le point principal.'],
  ['Nina likes most of a family dinner, but the final awkward toast is what she remembers.', 'Nina aime la plupart d un dîner de famille, mais le dernier toast gênant est ce dont elle se souvient.'],
  ['Eli gets helpful feedback on a draft, but the final correction is what he worries about most.', 'Eli reçoit des retours utiles sur un brouillon, mais la dernière correction est ce qui l inquiète le plus.'],
  ['Lina enjoys a local festival, but the final rain shower is the moment she defines the day by.', 'Lina aime un festival local, mais la dernière averse est le moment qui définit la journée pour elle.'],
  ['Grant has a productive call, but the closing "we need to talk later" is what he remembers.', 'Grant a un appel productif, mais le dernier "il faut qu on parle plus tard" est ce dont il se souvient.'],
  ['Tia follows a long set of class instructions, but the final warning about grading is all she can think about.', 'Tia suit une longue série d instructions de classe, mais le dernier avertissement sur la notation est tout ce à quoi elle pense.'],
  ['Finn enjoys a weekend outing, but the final traffic delay is the memory that sticks.', 'Finn apprécie une sortie de week-end, mais le dernier retard de circulation est le souvenir qui reste.'],
  ['Mina hears many nice things at a family gathering, but the last awkward joke is what dominates her memory.', 'Mina entend beaucoup de choses gentilles à une réunion de famille, mais la dernière blague gênante domine son souvenir.'],
  ['Theo reads a long update with lots of good news, but the final note about delay is what he focuses on.', 'Théo lit une longue mise à jour avec beaucoup de bonnes nouvelles, mais la dernière note sur le retard est ce sur quoi il se concentre.'],
  ['Juno enjoys a seminar with many examples, but the final anecdote is the one she remembers later.', 'Juno apprécie un séminaire avec beaucoup d exemples, mais la dernière anecdote est celle dont elle se souvient plus tard.'],
  ['Iris gets a clean bill from the mechanic, but the final estimate for a minor fix is what she dwells on.', 'Iris obtient un bon diagnostic du mécanicien, mais le dernier devis pour une petite réparation est ce sur quoi elle s attarde.'],
  ['Cole enjoys a dinner party, but the closing awkward toast is the part he retells.', 'Cole apprécie un dîner entre amis, mais le toast final gênant est la partie qu il raconte à nouveau.'],
  ['Sophie reads a long helpful thread, but the final joke is what she remembers after closing it.', 'Sophie lit un long fil utile, mais la dernière blague est ce dont elle se souvient après l avoir fermé.'],
];
