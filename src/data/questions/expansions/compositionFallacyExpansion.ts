...
const SCENARIOS: ReadonlyArray<readonly [string, string]> = [
  ...
  ['Each drawer in the desk opens smoothly, so the whole desk must be indestructible.', 'Chaque tiroir du bureau s ouvre bien, donc tout le bureau doit etre indestructible.'],
  ['Every cookie in the box tastes good, so the whole batch must be the best dessert ever.', 'Chaque biscuit de la boite est bon, donc toute la fournee doit etre le meilleur dessert de tous les temps.'],
  ['The car has a clean windshield and a strong radio, so the whole car must be perfect.', 'La voiture a un pare-brise propre et une radio puissante, donc toute la voiture doit etre parfaite.'],
  ['Each student in the group can solve one puzzle, so the group must be able to solve anything instantly.', 'Chaque eleve du groupe peut resoudre une enigme, donc le groupe doit pouvoir tout resoudre instantanement.'],
  ['The office has good coffee and comfy chairs, so the entire company must be great at everything.', 'Le bureau a un bon cafe et des chaises confortables, donc toute l entreprise doit etre excellente en tout.'],
  ['Every brick is solid, so the wall can never crack.', 'Chaque brique est solide, donc le mur ne peut jamais se fissurer.'],
  ['The salad has fresh lettuce and ripe tomatoes, so the whole buffet is automatically healthy.', 'La salade a de la laitue fraiche et des tomates mures, donc tout le buffet est automatiquement sain.'],
  ['Each part of the playlist sounds good, so the full party soundtrack must fit any mood.', 'Chaque morceau de la playlist sonne bien, donc toute la bande-son de la fete doit convenir a n importe quelle humeur.'],
  ['The laptop has a fast keyboard and bright screen, so the whole machine is unstoppable.', 'L ordinateur a un clavier rapide et un ecran lumineux, donc toute la machine est inarretable.'],
  ['Every seat cushion is soft, so the whole theater is a luxury experience.', 'Chaque coussin de siege est doux, donc tout le theatre est une experience de luxe.'],
  ['The sneakers are each light, so the pair must make you run like a pro.', 'Les baskets sont chacune legeres, donc la paire doit vous faire courir comme un pro.'],
  ['Every room in the house is tidy, so the house must be perfect for hosting royalty.', 'Chaque piece de la maison est rangee, donc la maison doit etre parfaite pour recevoir de la royaute.'],
  ['The phone has a good camera and a clear speaker, so the phone can handle anything.', 'Le telephone a une bonne camera et un haut-parleur clair, donc le telephone peut tout gerer.'],
  ['Each chapter is short, so the book must be easy for everyone to finish in one sitting.', 'Chaque chapitre est court, donc le livre doit etre facile a finir pour tout le monde en une seule fois.'],
  ['The bakery smells great and the display is neat, so every pastry must be gourmet.', 'La boulangerie sent bon et la vitrine est nette, donc chaque patisserie doit etre gastronomique.'],
  ['Every wheel spins well, so the bike cannot ever fail on a trip.', 'Chaque roue tourne bien, donc le velo ne peut jamais tomber en panne pendant un trajet.'],
  ['The concert had good sound and bright lights, so the whole band must have no weak songs.', 'Le concert avait un bon son et de belles lumieres, donc tout le groupe ne doit avoir aucune chanson faible.'],
  ['Each ingredient is fresh, so the stew must be delicious no matter how it is cooked.', 'Chaque ingredient est frais, donc le ragoût doit etre delicieux quelle que soit la maniere de le cuisiner.'],
  ['The class has smart students, so the class as a whole must win every contest.', 'La classe a des eleves intelligents, donc toute la classe doit gagner chaque concours.'],
  ['Every panel on the fence is painted neatly, so the whole fence must be strong.', 'Chaque panneau de la cloture est peint proprement, donc toute la cloture doit etre solide.'],
  ['The couch is soft and the lamp is bright, so the whole apartment must be luxurious.', 'Le canape est doux et la lampe est lumineuse, donc tout l appartement doit etre luxueux.'],
  ['Each song on the playlist is calm, so the whole party must stay calm too.', 'Chaque chanson de la playlist est calme, donc toute la fete doit rester calme aussi.'],
  ['The dessert has sweet cream and fresh fruit, so the whole meal is automatically healthy.', 'Le dessert a de la creme sucree et des fruits frais, donc tout le repas est automatiquement sain.'],
  ['Every tool in the box works, so the box can repair anything.', 'Chaque outil dans la boite fonctionne, donc la boite peut tout reparer.'],
  ['The garden has healthy roses and healthy herbs, so the whole yard is indestructible.', 'Le jardin a des roses en bonne sante et des herbes en bonne sante, donc tout le terrain est indestructible.'],
  ['Each person on the team is polite, so the team can never have conflict.', 'Chaque personne de l equipe est polie, donc l equipe ne peut jamais avoir de conflit.'],
  ['The menu has one healthy option and one tasty option, so the whole restaurant must be perfect.', 'Le menu a une option saine et une option savoureuse, donc tout le restaurant doit etre parfait.'],
  ['Every file opens fast, so the laptop is clearly top tier.', 'Chaque fichier s ouvre vite, donc l ordinateur portable est clairement de premier ordre.'],
  ['The box has two strong corners, so the entire box is unbreakable.', 'La boite a deux coins solides, donc toute la boite est incassable.'],
  ['Every slice of pizza tastes good, so the whole pizza place must be amazing.', 'Chaque part de pizza est bonne, donc toute la pizzeria doit etre formidable.'],
];
