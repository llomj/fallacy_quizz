...
const SCENARIOS: ReadonlyArray<readonly [string, string]> = [
  ...
  ['A bakery ad says only a cold person would skip this cupcake, so Maya buys it even though she was not hungry.', 'La pub de la boulangerie dit que seule une personne froide sauterait ce cupcake, alors Maya l achète même si elle n avait pas faim.'],
  ['A text says the rescue ride will leave a child alone if you do not come, so Leo says yes without checking the details.', 'Un texto dit que la course de secours laissera un enfant seul si vous ne venez pas, alors Leo accepte sans verifier les details.'],
  ['A poster shows a teary puppy and says this is your one chance to be kind, so Nina donates on the spot.', "Une affiche montre un chiot en larmes et dit que c est votre seule chance d être gentil, alors Nina donne sur-le-champ."],
  ['A sale banner says your family deserves comfort after a hard week, so Omar orders the expensive blanket.', 'Une bannière de vente dit que votre famille merite du confort apres une semaine difficile, alors Omar commande la couverture chere.'],
  ['A warning says if you ignore this alert you will feel guilty forever, so Priya clicks right away.', 'Un avertissement dit que si vous ignorez cette alerte vous vous sentirez coupable pour toujours, alors Priya clique aussitot.'],
  ['A charity video says imagine your own child in this photo, so Ben gives before reading the mission.', 'Une vidéo caritative dit d imaginer votre propre enfant sur cette photo, alors Ben donne avant de lire la mission.'],
  ['A parent says good kids do not question this favor, so Tara agrees even though it sounds unfair.', "Un parent dit que les bons enfants ne remettent pas en question ce service, alors Tara accepte meme si cela semble injuste."],
  ['A message says brave people act now, so Kai accepts the risky plan without thinking it through.', 'Un message dit que les personnes courageuses agissent maintenant, alors Kai accepte le plan risqué sans y réfléchir.'],
  ['A social post says the whole town is heartbroken about this cause, so Jules shares it without checking if the story is true.', "Une publication dit que toute la ville est bouleversée par cette cause, alors Jules la partage sans verifier si l histoire est vraie."],
  ['A seller says only selfish people ask questions, so Ava stops comparing prices and buys fast.', 'Un vendeur dit que seules les personnes egoistes posent des questions, alors Ava cesse de comparer les prix et achete vite.'],
  ['A flyer says your future self will thank you forever, so Noah signs up for the course on impulse.', 'Un prospectus dit que votre futur vous vous remerciera pour toujours, alors Noah s inscrit au cours sur un coup de tete.'],
  ['A fundraising message says the children are counting on you tonight, so Rina donates without reading more.', 'Un message de collecte dit que les enfants comptent sur vous ce soir, alors Rina donne sans lire davantage.'],
  ['A trailer with sad music says the movie will heal your heart, so Hugo assumes it must be meaningful.', 'Une bande-annonce avec musique triste dit que le film guerira votre coeur, alors Hugo suppose qu il doit etre profond.'],
  ['A warning says the wrong choice will embarrass your whole family, so Lina changes her answer quickly.', 'Un avertissement dit que le mauvais choix embarrassera toute votre famille, alors Lina change vite sa réponse.'],
  ['A text says decent people help now, so Grant says yes even though he is busy.', 'Un texto dit que les personnes decentes aident maintenant, alors Grant dit oui meme s il est occupe.'],
  ['A donation page says this is your chance to prove you care, so Zoe gives money before reading the cause.', 'Une page de don dit que c est votre chance de prouver que vous vous souciez des autres, alors Zoe donne de l argent avant de lire la cause.'],
  ['A commercial says the puppy will be lonely if you do not subscribe, so Milo clicks the button.', 'Une pub dit que le chiot sera seul si vous ne vous abonnez pas, alors Milo clique sur le bouton.'],
  ['A message says saying no would be cruel, so Theo accepts the request and feels bad about it.', 'Un message dit que dire non serait cruel, alors Theo accepte la demande et se sent mal.'],
  ['A poster says the campaign is the kind thing to support, so Pia agrees even though the facts are fuzzy.', 'Une affiche dit que soutenir cette campagne est la chose gentille a faire, alors Pia accepte meme si les faits sont flous.'],
  ['A video uses sad piano and close-up tears, so Sam thinks the claim must be sincere.', 'Une video utilise un piano triste et des gros plans de larmes, alors Sam pense que l affirmation doit etre sincere.'],
  ['A boss says only a bad teammate would leave now, so Mia stays late out of guilt.', 'Un chef dit que seul un mauvais coéquipier partirait maintenant, alors Mia reste tard par culpabilite.'],
  ['A sale note says the deal is your last chance to do something for your family, so Omar buys quickly.', 'Une note de vente dit que l offre est votre derniere chance de faire quelque chose pour votre famille, alors Omar achete vite.'],
  ['A charity pitch says think of the children crying tonight, so Priya gives without checking.', 'Une présentation caritative dit de penser aux enfants qui pleureront ce soir, alors Priya donne sans verifier.'],
  ['A family member tears up and says refusing would be cruel, so Ben says yes.', 'Un membre de la famille a les larmes aux yeux et dit que refuser serait cruel, alors Ben dit oui.'],
  ['A speaker says real compassion means saying yes, so Hugo agrees and feels relieved.', 'Un orateur dit que la vraie compassion, c est dire oui, alors Hugo accepte et se sent soulagé.'],
  ['A commercial says the blanket will protect your family from sadness, so Ava orders one.', 'Une publicité dit que la couverture protégera votre famille de la tristesse, alors Ava en commande une.'],
  ['A post says the tragic story proves you should help, so Milo helps without checking the facts.', 'Une publication dit que l histoire tragique prouve que vous devez aider, alors Milo aide sans verifier les faits.'],
  ['A message says this is your chance to be one of the good ones, so Nora signs up.', 'Un message dit que c est votre chance de faire partie des bons, alors Nora s inscrit.'],
  ['A banner says the children need you to act now, so Kai responds before verifying the details.', 'Une bannière dit que les enfants ont besoin que vous agissiez maintenant, alors Kai répond avant de verifier les details.'],
  ['A warning says leaving now would make you look heartless, so Jules stays.', 'Un avertissement dit que partir maintenant vous ferait paraitre sans coeur, alors Jules reste.'],
  ['A charity ad says every caring person helps today, so Rina contributes right away.', 'Une pub caritative dit que toute personne attentionnee aide aujourd hui, alors Rina contribue immediatement.'],
  ['A social clip says the tears are real so the product must matter, so Maya trusts it.', 'Un clip social dit que les larmes sont vraies donc le produit doit compter, alors Maya y croit.'],
  ['A plea says your kindness could save someone tonight, so Noah gives before thinking.', 'Un appel dit que votre gentillesse pourrait sauver quelqu un ce soir, alors Noah donne avant de réfléchir.'],
  ['A banner says no loving person would walk past this need, so Priya stops to help.', 'Une bannière dit qu aucune personne aimante ne passerait devant ce besoin, alors Priya s arrete pour aider.'],
  ['A message says the only decent response is to buy now, so Ben clicks purchase.', 'Un message dit que la seule réponse decente est d acheter maintenant, alors Ben clique sur acheter.'],
  ['A poster says your community is begging you to help, so Tara volunteers without reading the plan.', 'Une affiche dit que votre communauté vous supplie d aider, alors Tara se porte volontaire sans lire le plan.'],
  ...
  ['A commercial says this purchase will make your whole house feel loved, so Sam buys it.', 'Une pub dit que cet achat fera sentir toute votre maison aimee, alors Sam l achete.'],
  ['A flyer says good friends do not hesitate, so Eli says yes out of guilt.', 'Un prospectus dit que les vrais amis n hesitent pas, alors Eli dit oui par culpabilité.'],
  ['A donation request says your heart will regret saying no, so Mia contributes.', 'Une demande de don dit que votre coeur regrettera de dire non, alors Mia contribue.'],
  ['A headline says this is your last chance to be brave, so Omar clicks without thinking.', 'Un titre dit que c est votre derniere chance d etre courageux, alors Omar clique sans réfléchir.'],
  ['A seller says the premium version is what generous people choose, so Lina pays extra.', 'Un vendeur dit que la version premium est ce que choisissent les personnes genereuses, alors Lina paie plus.'],
  ['A charity page says imagine your own child in this pain, so Grant donates on emotion alone.', 'Une page caritative dit d imaginer votre propre enfant dans cette douleur, alors Grant donne sous le coup de l emotion.'],
  ['A message says brave people do not hesitate, so Zoe accepts the risky change.', 'Un message dit que les personnes courageuses n hesitent pas, alors Zoe accepte le changement risqué.'],
  ['A trailer uses tragic strings and slow motion, so Theo thinks the movie must be meaningful.', 'Une bande-annonce utilise des cordes tragiques et du ralenti, alors Theo pense que le film doit etre profond.'],
  ['A social post says the video is heartbreaking, so Pia believes the claim without checking it.', 'Une publication dit que la video est bouleversante, alors Pia croit l affirmation sans la verifier.'],
];
