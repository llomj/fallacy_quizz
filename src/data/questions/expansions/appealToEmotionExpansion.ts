...
const SCENARIOS: ReadonlyArray<readonly [string, string]> = [
  ['A charity ad says your child will cry in the rain if you do not donate, so Mia gives money right away.', 'Une pub caritative dit que votre enfant pleurera sous la pluie si vous ne donnez pas, alors Mia donne tout de suite.'],
  ['A sales email says only a cold person would ignore this deal on pet sweaters, so Leo clicks buy.', 'Un courriel de vente dit que seule une personne froide ignorerait cette offre de pulls pour animaux, alors Leo clique sur acheter.'],
