...
const SCENARIOS: ReadonlyArray<readonly [string, string]> = [
  ...
  ['Maya says the homework is either done or not done, so a half-finished page is impossible.', "Maya dit que les devoirs sont soit faits soit non faits, donc une page à moitié terminée est impossible."],
  ['Theo says the ride was either smooth or terrible, so a few bumps do not count as real travel.', "Theo dit que le trajet était soit fluide soit terrible, donc quelques secousses ne comptent pas comme un vrai voyage."],
  ['Nina says the jacket is either stylish or ugly, so simple and neat is ruled out.', "Nina dit que la veste est soit élégante soit laide, donc simple et soignée est exclue."],
  ['Ben says the pizza is either perfect or ruined, so a tasty but imperfect slice does not exist.', "Ben dit que la pizza est soit parfaite soit ratée, donc une part savoureuse mais imparfaite n'existe pas."],
  ['Priya says the city is either safe or unsafe, so safer and riskier neighborhoods inside it are ignored.', "Priya dit que la ville est soit sûre soit dangereuse, donc les quartiers plus sûrs ou plus risqués à l'intérieur sont ignorés."],
  ['Omar says the teacher is either strict or lazy, so fair and firm is not an option.', "Omar dit que le professeur est soit strict soit paresseux, donc juste et ferme n'est pas une option."],
  ['Ava says the game is either fun or boring, so mixed feelings are treated as nonsense.', "Ava dit que le jeu est soit amusant soit ennuyeux, donc les sentiments mitigés sont traités comme absurdes."],
  ['Leo says the meeting was either productive or wasted, so partly useful discussion is dismissed.', "Leo dit que la réunion était soit productive soit perdue, donc une discussion partiellement utile est rejetée."],
  ['Rina says the route is either shortest or wrong, so a scenic but slightly longer route is excluded.', "Rina dit que l'itinéraire est soit le plus court soit faux, donc un itinéraire pittoresque mais un peu plus long est exclu."],
  ['Finn says the lunch was either delicious or disgusting, so an okay meal is not allowed.', "Finn dit que le déjeuner était soit délicieux soit dégoûtant, donc un repas correct n'est pas permis."],
  ['Lina says the budget is either huge or tiny, so a normal-sized budget cannot exist.', "Lina dit que le budget est soit énorme soit minuscule, donc un budget de taille normale ne peut pas exister."],
  ['Eli says the app is either brilliant or useless, so pretty good gets ignored.', "Eli dit que l'application est soit géniale soit inutile, donc plutôt bonne est ignorée."],
  ['Tara says the plan is either smart or stupid, so practical but imperfect is not real.', "Tara dit que le plan est soit intelligent soit stupide, donc pratique mais imparfait n'est pas réel."],
  ['Noah says the party was either a hit or a flop, so moderate success does not count.', "Noah dit que la fête était soit un succès soit un flop, donc un succès modéré ne compte pas."],
  ['Mila says the phone is either new or old, so recently repaired does not fit.', "Mila dit que le téléphone est soit neuf soit vieux, donc récemment réparé ne convient pas."],
  ['Kai says the movie is either amazing or bad, so pretty good is brushed aside.', "Kai dit que le film est soit incroyable soit mauvais, donc plutôt bon est balayé."],
  ['Aria says the chair is either comfortable or awful, so decent but not amazing is dismissed.', "Aria dit que la chaise est soit confortable soit horrible, donc correcte mais pas incroyable est rejetée."],
  ['Owen says the schedule is either perfect or ruined, so just okay is not an answer.', "Owen dit que l'emploi du temps est soit parfait soit ruiné, donc simplement correct n'est pas une réponse."],
  ['Zoe says the food is either healthy or unhealthy, so a somewhat healthy meal is ignored.', "Zoe dit que la nourriture est soit saine soit malsaine, donc un repas un peu sain est ignoré."],
  ['Milo says the car is either reliable or broken, so a few quirks do not matter.', "Milo dit que la voiture est soit fiable soit cassée, donc quelques bizarreries ne comptent pas."],
  ['Mina says the meeting is either yes or no, so maybe is treated like a mistake.', "Mina dit que la réunion est soit oui soit non, donc peut-être est traité comme une erreur."],
  ['Jin says the house is either clean or filthy, so lived-in and tidy is excluded.', "Jin dit que la maison est soit propre soit sale, donc habité mais rangé est exclu."],
  ['Sofia says the route is either open or closed, so partial closures cannot happen.', "Sofia dit que la route est soit ouverte soit fermée, donc les fermetures partielles ne peuvent pas exister."],
  ['Dan says the policy is either kind or cruel, so mixed motives vanish.', "Dan dit que la politique est soit gentille soit cruelle, donc les motifs mixtes disparaissent."],
  ['Lea says the answer is either clear or confusing, so slightly unclear is not real.', "Lea dit que la réponse est soit claire soit confuse, donc un peu floue n'est pas réelle."],
  ['Rosa says the apartment is either tiny or huge, so normal size gets ignored.', "Rosa dit que l'appartement est soit minuscule soit immense, donc une taille normale est ignorée."],
  ['Hugo says the trip was either smooth or disastrous, so a few delays do not fit.', "Hugo dit que le voyage était soit fluide soit désastreux, donc quelques retards ne rentrent pas."],
  ['Nora says the message was either polite or rude, so direct but kind is excluded.', "Nora dit que le message était soit poli soit grossier, donc direct mais aimable est exclu."],
  ['Theo says the school is either excellent or awful, so average reviews are nonsense.', "Theo dit que l'école est soit excellente soit affreuse, donc les avis moyens sont absurdes."],
  ['Aisha says the book is either gripping or dull, so thoughtful but slow is impossible.', "Aisha dit que le livre est soit captivant soit ennuyeux, donc réfléchi mais lent est impossible."],
  ['Ben says the app is either useful or useless, so somewhat helpful is not allowed.', "Ben dit que l'application est soit utile soit inutile, donc un peu utile n'est pas permis."],
  ['Priya says the lesson is either easy or impossible, so hard but fair is rejected.', "Priya dit que la leçon est soit facile soit impossible, donc difficile mais juste est rejetée."],
  ['Maya says the restaurant is either fancy or cheap, so mid-range spots do not exist.', "Maya dit que le restaurant est soit chic soit bon marché, donc les restaurants de milieu de gamme n'existent pas."],
  ['Leo says the laptop is either new or old, so refurbished does not count.', "Leo dit que l'ordinateur portable est soit neuf soit vieux, donc reconditionné ne compte pas."],
  ['Rina says the day was either great or terrible, so ordinary good is ignored.', "Rina dit que la journée était soit super soit terrible, donc simplement bonne est ignorée."],
  ['Finn says the jacket is either stylish or ugly, so plain is not valid.', "Finn dit que la veste est soit élégante soit laide, donc simple n'est pas valide."],
  ['Milo says the workout is either painless or impossible, so hard but manageable is dismissed.', "Milo dit que l'entraînement est soit sans douleur soit impossible, donc difficile mais gérable est écarté."],
  ['Lina says the event is either a win or a waste, so useful but small is excluded.', "Lina dit que l'événement est soit une victoire soit une perte, donc utile mais petit est exclu."],
  ['Eli says the song is either catchy or boring, so quietly enjoyable is treated as nonsense.', "Eli dit que la chanson est soit entraînante soit ennuyeuse, donc discrètement agréable est traité comme absurde."],
  ['Tara says the rule is either fair or unfair, so mostly fair is dismissed.', "Tara dit que la règle est soit juste soit injuste, donc plutôt juste est rejetée."],
  ['Noah says the car is either safe or dangerous, so safer and riskier parts do not matter.', "Noah dit que la voiture est soit sûre soit dangereuse, donc les parties plus sûres et plus risquées ne comptent pas."],
  ['Zoe says the hotel is either luxurious or awful, so comfortable and basic is excluded.', "Zoe dit que l'hôtel est soit luxueux soit affreux, donc confortable et simple est exclu."],
  ['Omar says the lunch is either delicious or disgusting, so pleasantly okay is not a choice.', "Omar dit que le déjeuner est soit délicieux soit dégoûtant, donc agréablement correct n'est pas un choix."],
  ['Ava says the road is either short or wrong, so scenic but longer is rejected.', "Ava dit que la route est soit courte soit fausse, donc pittoresque mais plus longue est rejetée."],
  ['Kai says the chair is either comfortable or awful, so decent but not amazing is left out.', "Kai dit que la chaise est soit confortable soit horrible, donc correcte mais pas incroyable est laissée de côté."],
  ['Jules says the team is either brilliant or stupid, so practical but imperfect is ruled out.', "Jules dit que l'équipe est soit brillante soit stupide, donc pratique mais imparfaite est exclue."],
  ['Mina says the dinner is either expensive or cheap, so fairly priced is not real.', "Mina dit que le dîner est soit cher soit bon marché, donc à prix juste n'est pas réel."],
  ['Aria says the presentation is either amazing or terrible, so competent does not count.', "Aria dit que la présentation est soit incroyable soit terrible, donc compétente ne compte pas."],
  ['Nina says the city is either lively or dead, so calm but active is ignored.', "Nina dit que la ville est soit animée soit morte, donc calme mais active est ignoré."],
  ['Priya says the schedule is either perfect or ruined, so okay is not an option.', "Priya dit que l'emploi du temps est soit parfait soit ruiné, donc correct n'est pas une option."],
