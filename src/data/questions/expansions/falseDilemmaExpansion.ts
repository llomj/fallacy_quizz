...
const SCENARIOS: ReadonlyArray<readonly [string, string]> = [
  ...
  ['Zoe says the app is either revolutionary or worthless, so useful but imperfect software is dismissed.', "Zoe dit que l'application est soit revolutionnaire soit sans valeur, donc les logiciels utiles mais imparfaits sont rejetes."],
  ['Kai says the policy is either kind or cruel, so tradeoffs are treated as weakness.', "Kai dit que la politique est soit gentille soit cruelle, donc les compromis sont traites comme une faiblesse."],
  ['Maya says the game is either easy or impossible, so a tough but fair challenge is ignored.', "Maya dit que le jeu est soit facile soit impossible, donc un defi difficile mais equitable est ignore."],
  ['Nora says the meeting is either productive or wasted, so partly useful meetings disappear.', "Nora dit que la reunion est soit productive soit perdue, donc les reunions partiellement utiles disparaissent."],
  ['Grant says the route is either fastest or wrong, so scenic but sensible routes do not exist.', "Grant dit que l itineraire est soit le plus rapide soit faux, donc les routes pittoresques mais raisonnables n existent pas."],
  ['Juno says the book is either brilliant or unreadable, so average but enjoyable books vanish.', "Juno dit que le livre est soit brillant soit illisible, donc les livres moyens mais agreables disparaissent."],
  ['Pia says the plan is either bold or cowardly, so cautious confidence is erased.', "Pia dit que le plan est soit audacieux soit lâche, donc la prudence confiante est effacee."],
  ['Milo says the workout is either easy or impossible, so challenging but manageable is excluded.', "Milo dit que l entrainement est soit facile soit impossible, donc difficile mais gerable est exclu."],
  ['Lara says the city is either exciting or boring, so calm but pleasant places are forgotten.', "Lara dit que la ville est soit excitante soit ennuyeuse, donc les endroits calmes mais agreables sont oublies."],
  ['Ari says the project is either on time or a disaster, so slightly late but solid work is ignored.', "Ari dit que le projet est soit a l heure soit un desastre, donc le travail un peu en retard mais solide est ignore."],
  ['Hugo says the speaker is either charismatic or useless, so quiet competence is overlooked.', "Hugo dit que l orateur est soit charismatique soit inutile, donc la competence discrète est ignoree."],
  ['Iris says the school lunch is either delicious or inedible, so ordinary meals lose their place.', "Iris dit que le repas scolaire est soit delicieux soit immangeable, donc les repas ordinaires perdent leur place."],
  ['Cole says the schedule is either perfect or broken, so workable compromises disappear.', "Cole dit que l emploi du temps est soit parfait soit casse, donc les compromis utilisables disparaissent."],
  ['Evan says the phone plan is either amazing or a scam, so fair but basic options are excluded.', "Evan dit que le forfait telephone est soit incroyable soit une arnaque, donc les options justes mais basiques sont exclues."],
  ['Tessa says the art is either meaningful or pointless, so simple decoration is not allowed.', "Tessa dit que l art est soit significatif soit inutile, donc la decoration simple n est pas permise."],
  ['Mason says the road is either smooth or destroyed, so slightly bumpy roads are treated as failures.', "Mason dit que la route est soit lisse soit detruite, donc les routes un peu cahoteuses sont traitees comme des echecs."],
  ['Ray says the dog is either trained or hopeless, so still-learning progress is ignored.', "Ray dit que le chien est soit dresse soit sans espoir, donc les progres encore en cours sont ignores."],
  ['Stella says the party was either unforgettable or a waste, so pleasant but modest fun is overlooked.', "Stella dit que la fete etait soit inoubliable soit une perte de temps, donc le plaisir modeste mais agreable est ignore."],
  ['Jade says the policy is either freedom or oppression, so routine rules become dramatic.', "Jade dit que la politique est soit la liberte soit l oppression, donc les regles de routine deviennent dramatiques."],
  ['Drew says the class is either brilliant or hopeless, so average progress is invisible.', "Drew dit que la classe est soit brillante soit desesperee, donc les progres moyens sont invisibles."],
  ['Felix says the relationship is either perfect or broken, so normal work-in-progress relationships vanish.', "Felix dit que la relation est soit parfaite soit brisee, donc les relations ordinaires en evolution disparaissent."],
  ['Nora says the team is either focused or distracted, so mild off-topic moments are treated like failure.', "Nora dit que l equipe est soit concentree soit distraite, donc les petits ecarts de sujet sont traites comme un echec."],
  ['Oli says the restaurant is either cheap or expensive, so fair-priced meals do not count.', "Oli dit que le restaurant est soit bon marche soit cher, donc les repas a prix juste ne comptent pas."],
  ['Mia says the course is either inspiring or a failure, so merely solid teaching is erased.', "Mia dit que le cours est soit inspirant soit un echec, donc un enseignement simplement solide est efface."],
  ['Theo says the movie is either for adults or for kids, so teens are treated as invisible.', "Theo dit que le film est soit pour adultes soit pour enfants, donc les adolescents sont invisibles."],
  ['Ava says the event was either packed or empty, so a comfortable crowd is impossible.', "Ava dit que l evenement etait soit bondé soit vide, donc une foule confortable est impossible."],
  ['Ben says the feedback is either praise or hate, so useful criticism is ignored.', "Ben dit que le retour est soit un compliment soit de la haine, donc la critique utile est ignoree."],
  ['Rina says the game is either competitive or boring, so relaxed fun is impossible.', "Rina dit que le jeu est soit competitif soit ennuyeux, donc le plaisir detendu est impossible."],
  ['Leo says the dinner is either homemade or fake, so restaurant food does not count as real food.', "Leo dit que le dîner est soit fait maison soit faux, donc la nourriture du restaurant ne compte pas comme de la vraie nourriture."],
  ['Maya says the project is either a success or a failure, so partial success is ignored.', "Maya dit que le projet est soit un succes soit un echec, donc le succes partiel est ignore."],
  ['Theo says the commute is either by car or a waste of time, so biking is not considered.', "Theo dit que le trajet est soit en voiture soit une perte de temps, donc le velo n est pas envisage."],
  ['Nina says the restaurant is either fancy or bad, so a normal good meal disappears.', "Nina dit que le restaurant est soit chic soit mauvais, donc un repas normal mais bon disparait."],
  ['Owen says the school is either strict or lazy, so steady ordinary teaching is excluded.', "Owen dit que l ecole est soit stricte soit paresseuse, donc un enseignement ordinaire et regulier est exclu."]
];
