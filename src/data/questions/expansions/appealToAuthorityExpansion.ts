...
const SCENARIOS: ReadonlyArray<readonly [string, string]> = [
  ['A mechanic says this soup cures colds after hearing it once from a chef, so Maya adds it to her menu without checking the recipe.', 'Un mécanicien dit que cette soupe guérit les rhumes apres l avoir entendu une fois d un chef, alors Maya l ajoute a son menu sans verifier la recette.'],
  ['A dentist says this bike helmet is the best one for mountain climbing, so Leo buys it for his commute across flat streets.', 'Un dentiste dit que ce casque de velo est le meilleur pour l alpinisme, alors Leo l achete pour aller au travail sur des rues plates.'],
