...
const SCENARIOS: ReadonlyArray<readonly [string, string]> = [
  ...
  ['Zoe says the meal box should be rejected because one side dish is too spicy.', 'Zoé dit que la boite-repas doit etre rejetee parce qu un accompagnement est trop epice.'],
  ['Kai says the school welcome pack is bad because it includes both a map and a schedule.', 'Kai dit que le pack de bienvenue scolaire est mauvais parce qu il comprend une carte et un emploi du temps.'],
  ['Nora says the festival wristband is pointless because one coupon is for a food stall she would not visit.', 'Nora dit que le bracelet du festival ne sert à rien parce qu un coupon est pour un stand de nourriture qu elle ne visiterait pas.'],
  ['Eli says the software suite is terrible because one tool is harder to learn than the others.', 'Eli dit que la suite logicielle est terrible parce qu un outil est plus difficile à apprendre que les autres.'],
  ['Lina says the community plan should be rejected because one paragraph mentions parking.', 'Lina dit que le plan communautaire doit etre rejeté parce qu un paragraphe mentionne le stationnement.'],
  ['Grant says the group offer is bad because it bundles two useful things he wanted separately.', 'Grant dit que l offre groupée est mauvaise parce qu elle regroupe deux choses utiles qu il voulait séparément.'],
  ['Tia says the library pass is a bad deal because it comes with both borrowing and event access.', 'Tia dit que le pass bibliothèque est une mauvaise affaire parce qu il inclut à la fois l emprunt et l accès aux événements.'],
  ['Noah says the family package should be refused because one activity is not his favorite.', 'Noah dit que le forfait familial doit etre refusé parce qu une activité n est pas sa préférée.'],
  ['Mia says the volunteer starter kit is bad because it includes a badge and a notebook together.', 'Mia dit que le kit de démarrage pour bénévoles est mauvais parce qu il comprend un badge et un carnet ensemble.'],
  ['Jules says the housewarming bundle is bad because the candle scent is not his style.', 'Jules dit que le lot de crémaillère est mauvais parce que le parfum de la bougie n est pas son style.'],
  ['Ava says the club membership is worthless because one benefit is a newsletter she will not read.', 'Ava dit que l adhésion au club ne vaut rien parce qu un avantage est une newsletter qu elle ne lira pas.'],
  ['Finn says the event combo is bad because it includes both a talk and a workshop.', 'Finn dit que le combo événementiel est mauvais parce qu il inclut une conférence et un atelier.'],
  ['Mina says the school supply pack should be rejected because one notebook has a cover she finds ugly.', 'Mina dit que le pack de fournitures scolaires doit etre rejeté parce qu un cahier a une couverture qu elle trouve moche.'],
  ['Theo says the city offer is bad because it mixes transit passes with museum tickets.', 'Théo dit que l offre de la ville est mauvaise parce qu elle mélange des titres de transport et des billets de musée.'],
  ['Juno says the family bundle is useless because it includes a fruit snack box.', 'Juno dit que le lot familial est inutile parce qu il inclut une boîte de snacks aux fruits.'],
  ['Iris says the concert package is bad because one opening act is too quiet.', 'Iris dit que le forfait concert est mauvais parce qu un artiste d ouverture est trop calme.'],
  ['Cole says the office starter set should be thrown out because it has a pen he already owns.', 'Cole dit que le kit de bureau doit etre jeté parce qu il contient un stylo qu il possède déjà.'],
  ['Riley says the travel kit is bad because it includes a map and a charger.', 'Riley dit que le kit de voyage est mauvais parce qu il comprend une carte et un chargeur.'],
  ['Pia says the workshop bundle is bad because the bonus handout is printed on cheap paper.', 'Pia dit que le lot de l atelier est mauvais parce que le document bonus est imprimé sur du papier bon marché.'],
  ['Hugo says the school event package is a scam because it combines a ticket and a drink voucher.', 'Hugo dit que le pack de l événement scolaire est une arnaque parce qu il combine un billet et un bon boisson.'],
  ['Sam says the recipe pack should be refused because it includes both ingredients and instructions.', 'Sam dit que le pack de recettes doit etre refusé parce qu il comprend des ingrédients et des instructions.'],
  ['Jade says the club kit is bad because the mug is not microwave safe.', 'Jade dit que le kit du club est mauvais parce que la tasse ne va pas au micro-ondes.'],
  ['Owen says the student bundle is bad because one bookmark is too plain.', 'Owen dit que le pack étudiant est mauvais parce qu un marque-page est trop simple.'],
  ['Lea says the charity package is bad because it includes both a donation receipt and a sticker.', 'Léa dit que le forfait caritatif est mauvais parce qu il inclut un reçu de don et un autocollant.'],
  ['Maya says the sports fan pack is bad because one item is for a rival team.', 'Maya dit que le pack de fan de sport est mauvais parce qu un objet est pour une équipe rivale.'],
  ['Ben says the party bundle is a no because the playlist and the snacks come together.', 'Ben dit que le lot de fête est un non parce que la playlist et les snacks viennent ensemble.'],
  ['Ava says the app bundle is bad because it includes a tutorial she does not need.', 'Ava dit que le lot d applications est mauvais parce qu il comprend un tutoriel dont elle n a pas besoin.'],
  ['Omar says the family plan should be rejected because the calendar icon is ugly.', 'Omar dit que le forfait familial doit etre rejeté parce que l icône du calendrier est moche.'],
  ['Nina says the team kit is bad because the water bottle is the wrong color.', 'Nina dit que le kit d équipe est mauvais parce que la bouteille d eau a la mauvaise couleur.'],
  ['Eli says the neighborhood welcome pack is useless because one brochure is boring.', 'Eli dit que le pack de bienvenue du quartier est inutile parce qu une brochure est ennuyeuse.'],
  ['Lina says the subscription should be rejected because it includes both streaming and downloads.', 'Lina dit que l abonnement doit etre rejeté parce qu il inclut le streaming et les téléchargements.'],
  ['Grant says the road trip package is bad because the snack bag has raisins.', 'Grant dit que le forfait road trip est mauvais parce que le sachet de snacks contient des raisins secs.'],
  ['Tia says the school event pass is bad because it gives access to both the talk and the after party.', 'Tia dit que le pass de l événement scolaire est mauvais parce qu il donne accès à la conférence et à la soirée.'],
  ['Noah says the software bundle is bad because the help docs are too long.', 'Noah dit que le lot de logiciels est mauvais parce que la documentation d aide est trop longue.'],
  ['Mia says the retreat package is bad because it includes a yoga class she is not sure about.', 'Mia dit que le forfait retraite est mauvais parce qu il inclut un cours de yoga dont elle n est pas sûre.'],
  ['Jules says the welcome box is bad because the notebook cover is a color he dislikes.', 'Jules dit que la boîte de bienvenue est mauvaise parce que la couverture du carnet est d une couleur qu il n aime pas.'],
  ['Ava says the museum membership is a scam because the brochure and ticket are sold together.', 'Ava dit que l abonnement au musée est une arnaque parce que la brochure et le billet sont vendus ensemble.'],
  ['Finn says the gift set is bad because one candle smells like citrus.', 'Finn dit que le coffret cadeau est mauvais parce qu une bougie sent les agrumes.'],
  ['Mina says the class bundle is bad because it includes both extra reading and a quiz.', 'Mina dit que le pack de cours est mauvais parce qu il inclut une lecture supplémentaire et un quiz.'],
  ['Theo says the office welcome kit is bad because the pen clicks too loudly.', 'Théo dit que le kit de bienvenue du bureau est mauvais parce que le stylo clique trop fort.'],
  ['Juno says the concert pass is bad because the merch discount is only ten percent.', 'Juno dit que le pass concert est mauvais parce que la réduction sur le merchandising n est que de dix pour cent.'],
  ['Iris says the travel bundle is bad because one brochure lists a museum she will skip.', 'Iris dit que le lot de voyage est mauvais parce qu une brochure mentionne un musée qu elle sautera.'],
  ['Cole says the school starter pack is bad because it includes a ruler and a folder.', 'Cole dit que le pack de démarrage scolaire est mauvais parce qu il inclut une règle et un classeur.'],
  ['Riley says the family event package is bad because one song is old-fashioned.', 'Riley dit que le forfait événement familial est mauvais parce qu une chanson est démodée.'],
  ['Pia says the game night bundle is bad because the card deck has too many cards.', 'Pia dit que le lot de soirée jeu est mauvais parce que le paquet de cartes contient trop de cartes.'],
  ['Sam says the workshop bundle is bad because the notebook has lined paper.', 'Sam dit que le lot de l atelier est mauvais parce que le carnet a du papier ligné.'],
  ['Jade says the volunteer package is bad because it comes with a badge and a schedule.', 'Jade dit que le forfait bénévole est mauvais parce qu il vient avec un badge et un programme.'],
  ['Owen says the bundle is either perfect or worthless because one part is not ideal.', 'Owen dit que le lot est soit parfait soit sans valeur parce qu une partie n est pas idéale.'],
];
